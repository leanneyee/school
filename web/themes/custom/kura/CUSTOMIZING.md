...
**Don't subtheme Kura!** It does not provide backwards compatibility. This allows us to rapidly innovate, iterate, and improve it. If you create a sub-theme of Kura, it is likely to break in the future.
...
If you want to make deeper customizations (e.g., to components or JavaScript), you will need to convert Kura to a custom theme with the same machine name. You can do this by running the following at the command line, from the Drupal project root (assuming `web` is the web root):

```shell
mkdir -p web/themes/custom
cp -R web/themes/contrib/kura web/themes/custom/kura
git add web/themes/custom/kura
composer remove drupal/kura
```
...
Kura uses [single-directory components](https://www.drupal.org/docs/develop/theming-drupal/using-single-directory-components) and comes with a variety of commonly used components. You can add new components and modify existing ones, but be sure to rebuild the CSS when you make changes.

## Building CSS

Kura uses [Tailwind](https://tailwindcss.com) to simplify styling by using classes to compose designs directly in the markup.
...
Kura uses [Prettier](https://prettier.io) to automatically format code for consistency. The project is configured with plugins for Tailwind CSS and Twig templates.
...
All the objects created this way will be stored in a global variable so you can do stuff with them later. Since the `namespace` variable at the top of component.js is `kuraComponents`, you would find the Accordion's ComponentType instance at `window.kuraComponents.accordion`.

Furthermore, `window.kuraComponents.accordion.instances` is an array of all the ComponentInstance objects, and `window.kuraComponents.accordion.elements` is an array of all the component container elements.
