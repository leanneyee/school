# Kura Theme - Agent Rules

This document contains coding rules and conventions for the Kura theme that AI agents should follow when making changes.
...
**Note**: Even though Drupal core templates sometimes use `<div{{ attributes }}>`, this pattern is not allowed in Kura theme components.
...
```twig
{% include '@kura/components/icon/icon.twig' with {
  weight: 'bold',
  icon: icon
} %}
```
...
**Rule**: After completing any changes to the Kura theme, always run `npm run format` and `npm run build` to ensure code is properly formatted and the build artifacts are up to date.
...
**Note**: These commands should be run from the Kura theme directory (`web/themes/custom/kura/`). Running these commands ensures that:
